    ...
    try {
        if (interaction.isChatInputCommand()) {
            ...
            if (name === 'setup-ticket') {
                const isStaff = ticketService.memberHasAnyStaffRole(interaction.member);
                if (!isStaff) {
                    const roleIds = ticketService.getMemberRoleIds(interaction.member);
                ...
                await channel.send({ embeds: [embed], components: [...rows, ...selectRows] });
            ...
            if (name === 'close') {
                ...
                const closed = await ticketService.closeTicket(interaction.channel, interaction.user, reason);
                if (!closed) {
            ...
            if (name === 'adduser') {
                ...
                if (!ticketService.memberHasAnyStaffRole(interaction.member)) {
                    return interaction.reply({ ephemeral: true, content: 'Solo lo staff può aggiungere membri.' });
                }
                const user = interaction.options.getUser('user', true);
                const ok = await ticketService.addUserToTicket(interaction.channel, user);
                if (!ok) {
            ...
            if (name === 'removeuser') {
                ...
                const user = interaction.options.getUser('user', true);
                const ok = await ticketService.removeUserFromTicket(interaction.channel, user);
                if (!ok) {
                    return interaction.reply({ ephemeral: true, content: 'Canale non riconosciuto come ticket.' });
                }
        ...
        if (interaction.isButton() && interaction.customId === 'ticket_close') {
            ...
            const ok = await ticketService.closeTicket(interaction.channel, interaction.user, 'Chiusura tramite bottone');
            if (!ok) {
        ...
        if (interaction.isChatInputCommand() && interaction.commandName === 'panel') {
            ...
            if (sub === 'create') {
                ...
                const color = interaction.options.getInteger('embed_color') ?? configData.embedTheme.color;
                const footer = interaction.options.getString('embed_footer') ?? configData.embedTheme.footerText;
                const thumbnail = interaction.options.getString('embed_thumbnail') ?? '';
                ...
                if (!id || id.trim().length === 0)
                    errors.push('panel_id è obbligatorio.');
                if (title.length > 256)
                    errors.push('Il titolo non può superare 256 caratteri.');
                if (description.length > 4096)
                    errors.push('La descrizione non può superare 4096 caratteri.');
                if (color < 0 || color > 0xffffff)
                    errors.push('embed_color deve essere un intero tra 0 e 16777215.');
                if (errors.length) {
            ...
            if (sub === 'add-button') {
                ...
                panel.buttons = (panel.buttons ?? []).filter((b) => b.id !== button_id).concat([
            ...
            if (sub === 'remove-button') {
                ...
                if (!panel)
                    return interaction.reply({ ephemeral: true, content: `Pannello ${id} non trovato.` });
