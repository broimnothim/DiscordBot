import { ChannelType, PermissionFlagsBits, EmbedBuilder, AttachmentBuilder, ChannelType as DjsChannelType, PermissionsBitField } from 'discord.js';
import { ActionRowBuilder, ButtonBuilder, ButtonStyle } from 'discord.js';
import fs from 'fs-extra';
import path from 'path';
import { appendTicketMessage, logEvent, writeTranscriptHtml } from './Logger.js';
export class TicketService {
    constructor(client, config, dataDir) {
        this.rateLimitByUser = new Map();
        this.dynamicPanels = [];
        this.client = client;
        this.config = config;
        this.dataDir = dataDir;
    }
    memberHasAnyStaffRole(member) {
        if (!member)
            return false;
        const m = member;
        if (m.roles?.cache) {
            return m.roles.cache.some((r) => this.config.staffRoleIds.includes(r.id));
        }
        if (Array.isArray(m.roles)) {
            return m.roles.some((id) => this.config.staffRoleIds.includes(id));
        }
    ...
    async createTicket(interaction, preset) {
    ...
    async closeTicket(channel, executor, reason) {
        ...
        await logEvent(this.dataDir, 'close', `Ticket ${channel.id} chiuso da ${executor.tag} (opener ${entry.openerId})`);
    ...
    async trackMessageIfTicket(message) {
        const idx = await this.readIndex();
    ...
    async writeIndexEntry(entry) {
        const idx = await this.readIndex();
        idx.push(entry);
        await this.writeIndex(idx);
    }
    async getTicketEntry(channelId) {
        const idx = await this.readIndex();
    ...
    async resolveParentCategoryId(id, guild) {
        if (!id)
            return undefined;
        const target = await guild.channels.fetch(id).catch(() => null);
        if (!target)
            return undefined;
        if (target.type === DjsChannelType.GuildCategory)
            return target.id;
    ...
    async resolveNotifyChannel(id, guild) {
        if (!id)
            return null;
        const target = await guild.channels.fetch(id).catch(() => null);
        if (!target)
            return null;
        if (target.type === DjsChannelType.GuildText)
            return target;
        return null;
    }
}
