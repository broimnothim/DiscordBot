...
export class TicketService {
  ...
  async createTicket(interaction: Interaction, preset?: ButtonPreset) {
    ...
    channel = await guild.channels.create({
      ...
      const staffPings = this.config.staffRoleIds.map((id) => `<@&${id}>`).join(' ');
      const sent = await channel.send({ content: `<@${openerId}> ${staffPings}`, embeds: [welcome], components: [closeRow] });
